...
(function () {
  ...
  function ensureBoundsOverlay(zone) {
    const existing = zone.querySelector(".skill-game-bounds");
  ...
  function toLocalClamped(pageLeft, pageTop, w, h, zone) {
    ...
    const clampedLeft = Math.min(Math.max(pageLeft, zoneLeft),  maxLeft);
    const clampedTop  = Math.min(Math.max(pageTop,  zoneTop),   maxTop);

    return { localLeft: clampedLeft - zoneLeft, localTop: clampedTop - zoneTop };
  }

  function renderCloneAtLocalPos(x, y, clone) {
  ...
  function setExitButtons(visible) {
    const { top, bottom } = getExitButtons();
    const method = visible ? "remove" : "add";
    if (top) top.classList[method]("is-hidden");
    if (bottom) bottom.classList[method]("is-hidden");
  }
  ...
  function stepPhysics(dt) {
    ...
    for (const m of movers) {
      ...
      if (m.lp.x < 0) {
        m.lp.x = 0; m.vel.vx = -m.vel.vx * BOUNCE_WALL; triggerImpact(m.clone);
      } else if (m.lp.x + m.lp.w > zoneW) {
        m.lp.x = zoneW - m.lp.w; m.vel.vx = -m.vel.vx * BOUNCE_WALL; triggerImpact(m.clone);
      }

      if (m.lp.y < 0) {
        m.lp.y = 0; m.vel.vy = -m.vel.vy * BOUNCE_WALL; triggerImpact(m.clone);
      } else if (m.lp.y + m.lp.h > zoneH) {
        m.lp.y = zoneH - m.lp.h; m.vel.vy = -m.vel.vy * BOUNCE_WALL; triggerImpact(m.clone);
      }
    }
  ...
  function resolveAgainstDragged(lp, vel, dragRect, dragVX, dragVY, zoneW, zoneH, movingClone) {
    ...
    if (overlapX < overlapY) {
      if (lp.x + lp.w / 2 < dragRect.x + dragRect.w / 2) {
        lp.x -= overlapX; vel.vx = -Math.abs(dragVX) * KICK_SCALE; if (Math.abs(vel.vx) < MIN_KICK) vel.vx = -MIN_KICK;
      } else {
        lp.x += overlapX; vel.vx =  Math.abs(dragVX) * KICK_SCALE; if (Math.abs(vel.vx) < MIN_KICK) vel.vx =  MIN_KICK;
      }
      vel.vy += dragVY * 0.2;
      ...
      if (lp.y + lp.h / 2 < dragRect.y + dragRect.h / 2) {
        lp.y -= overlapY; vel.vy = -Math.abs(dragVY) * KICK_SCALE; if (Math.abs(vel.vy) < MIN_KICK) vel.vy = -MIN_KICK;
      } else {
        lp.y += overlapY; vel.vy =  Math.abs(dragVY) * KICK_SCALE; if (Math.abs(vel.vy) < MIN_KICK) vel.vy =  MIN_KICK;
      }
      vel.vx += dragVX * 0.2;
  ...
  function resolvePairwise(a, b, zoneW, zoneH) {
    ...
    if (overlapX < overlapY) {
      if (a.lp.x + a.lp.w / 2 < b.lp.x + b.lp.w / 2) { a.lp.x -= overlapX / 2; b.lp.x += overlapX / 2; }
      else                                            { a.lp.x += overlapX / 2; b.lp.x -= overlapX / 2; }
      const tmp = a.vel.vx; a.vel.vx = b.vel.vx * BOUNCE_TILE; b.vel.vx = tmp * BOUNCE_TILE;
    } else {
      if (a.lp.y + a.lp.h / 2 < b.lp.y + b.lp.h / 2) { a.lp.y -= overlapY / 2; b.lp.y += overlapY / 2; }
      else                                            { a.lp.y += overlapY / 2; b.lp.y -= overlapY / 2; }
      const tmp = a.vel.vy; a.vel.vy = b.vel.vy * BOUNCE_TILE; b.vel.vy = tmp * BOUNCE_TILE;
    }
  ...
  function realignAllClones() {
    ...
    const bounds = computeGameBounds();
    if (!bounds) return;
    state.zone = ensureGameZone(bounds);
    const zr = state.zone.getBoundingClientRect();
    const zoneW = zr.width;
    const zoneH = zr.height;
